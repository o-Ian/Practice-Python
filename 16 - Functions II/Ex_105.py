def notas(*nums, sit=False):
    ...
    :param sit: [opcional] Se True, a situação em que o aluno está é indicada
    :return: Dicionário com todas as informações
    """

    infos = {'qntd_notas': len(nums), 'maior nota': max(nums),
             'menor nota': min(nums), 'media_notas': sum(nums)/len(nums)}
    if sit:
